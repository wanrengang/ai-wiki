...
sources: [raw/articles/2026-05-14-triattention-kv-compression-nvidia-mit-zhejiang.md, raw/articles/2026-05-15-triattention-nvidia-mit-zju.md, raw/articles/2026-05-18-triattention-nvidia-mit-zju-kv-compression.md]
confidence: high
---
...
- **vLLM 插件**：挂上就能用，不需要改模型架构或重新训练。提供 OpenAI 兼容 API 的 server mode、Python API 以及 OpenClaw 接入说明
- **Apple Silicon**：MLX 实验性支持文档，覆盖 M1-M4 全系芯片，基于 mlx-lm 框架，但尚不成熟
- **两种路线可叠加**：量化派（TurboQuant）和选择性保留派（TriAttention）理论上可以同时使用，尚未有严格的 head-to-head 对比

## 消费级硬件部署里程碑

| 里程碑 | 变化 |
|--------|------|
| 1年前 | 本地跑大模型是极客行为艺术，7B 都要折腾半天 |
| 现在 | 32B 模型在单张消费级卡上完成完整 agent 任务 |
| 意义 | KV 压缩从消融实验变成每个开发者都能用的工程现实 |
